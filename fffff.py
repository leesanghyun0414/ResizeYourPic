...
for path in glob.glob(os.path.join(dir_path, f'*.{read_file_type}')):
    ...
    # すでにフォルダーが存在するか判定する
    if not os.path.exists(dir_path + file_name.split("スキルソウル")[0]):
    ...
    elif os.path.exists(dir_path + file_name.split("スキルソウル")[0]) and not os.path.exists(
            dir_path + file_name.split("スキルソウル")[0] + "/" + "スキルソウル"):
    ...
    for folder_name, img_size in size_dict.items():
        if 'アップ' not in file_name and folder_name == '640x640':
            ...
            if (not os.path.exists(dir_path + file_name + "/" + folder_name)) and "スキルソウル" not in file_name:
                os.mkdir(dir_path + file_name + "/" + folder_name)
                sizing_dir_path = dir_path + file_name + "/" + folder_name
                ...
                if int(file_num) < 10000:
                    ...
                    cv2_jp.imwrite(sizing_dir_path + "/" + "00" + file_num + '.png', img)
                elif int(file_num) < 100:
                    cv2_jp.imwrite(sizing_dir_path + "/" + "000" + file_num + '.png', img)
                elif int(file_num) < 10:
                    cv2_jp.imwrite(sizing_dir_path + "/" + "0000" + file_num + '.png', img)
                else:
                    cv2_jp.imwrite(sizing_dir_path + "/" + file_num + '.png', img)
